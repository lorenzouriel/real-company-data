...
- **`14330` default is a Windows-dev-only workaround.** `docker-compose.yml` maps SQL Server to host port `14330` (not `1433`) by default to dodge a common conflict with a native SQL Server install on Windows dev machines. On Linux hosts (including the home-lab/Tailscale deploy in `DEPLOY.md`) that conflict doesn't exist — set `SQLSERVER_PORT=1433` there.
- **DB ports must bind to a specific interface, never `0.0.0.0`.** `POSTGRES_BIND`/`SQLSERVER_BIND` in `.env` control which interface Docker publishes the port on. For the home-lab deploy this should be the host's Tailscale IP (`tailscale ip -4`) — that's the entire access-control mechanism, there's no firewall layered on top of it. Binding to `0.0.0.0` would expose the DB to the LAN (or further) with nothing enforcing the intended boundary.
